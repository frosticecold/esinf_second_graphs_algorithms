...
public class EdgeAsDoubleGraphAlgorithmsTest {
    ...
    public void testMinDistGraph() {
        AdjacencyMatrixGraph<String, Double> map = new AdjacencyMatrixGraph<>();

        map.insertVertex("Porto");
        map.insertVertex("Braga");
        map.insertVertex("Guimarães");
        map.insertVertex("Aveiro");
        map.insertVertex("Lisboa");
        map.insertVertex("Portimão");

        map.insertEdge("Porto", "Braga", 35.0);
        map.insertEdge("Porto", "Aveiro", 40.0);
        map.insertEdge("Porto", "Lisboa", 350.0);

        map.insertEdge("Lisboa", "Portimão", 150.0);

        map.insertEdge("Braga", "Guimarães", 30.0);

        AdjacencyMatrixGraph<String, Double> mapCaminhosCurtos = EdgeAsDoubleGraphAlgorithms.minDistGraph(map);
        System.out.println(mapCaminhosCurtos.toString());
